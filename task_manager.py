...
class Menu:
    ...
    def set_priority(self):
        print("Выберите приоритет:", "3 - Задача высокой важности", "2 - Задача умеренной важности",
              "1 - Задача низкой важности", sep="\n")
        while True:
            priority = input(">> ")
            if priority == "1":
    ...
    def tasks_sort(self, user_input):
        if user_input == "1":
            ...
            try:
                self.task_manager.tasks.sort(key=lambda task: datetime.strptime(task.date, "%Y-%m-%d (Время: %H:%M)"),
                                             reverse=True)
                self.task_manager.show_tasks()
            except ValueError:
            ...
            try:
                self.task_manager.tasks.sort(
                    key=lambda task: datetime.strptime(task.deadline, "%Y-%m-%d (Время: %H:%M)"), reverse=False)
                self.task_manager.show_tasks()
            except ValueError:
            ...
            if action == '1':
                ...
                if len(self.task_manager.tasks) > 0:
                    ...
                    if priority_change_choice <= len(self.task_manager.tasks):
                        ...
                        print(
                            f"Приоритет задачи '{self.task_manager.tasks[priority_change_choice - 1].title}' успешно изменен ")
                    else:
                ...
                if len(self.task_manager.tasks) > 0:
                    ...
                    if status_change_choice <= len(self.task_manager.tasks):
                        ...
                        print(
                            f"Статус задачи '{self.task_manager.tasks[status_change_choice - 1].title}' успешно изменен")
                    else:
                        print("Нет задачи с таким номером")
                else:
                    print("Список задач пуст (добавьте задачу)")
